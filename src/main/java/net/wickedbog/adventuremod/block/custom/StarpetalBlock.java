...
public class StarpetalBlock extends FlowerBlock {
    public StarpetalBlock(Holder<MobEffect> effect, float seconds, Properties properties) {
    ...
    public void animateTick(BlockState blockState, Level level, BlockPos pos, RandomSource random) {
        ...
        for (int i = 0; i < 3; ++i) {
            if(random.nextBoolean()) {
                LodestoneParticleEffects.fireflyParticles(level, d0 + random.nextDouble() / 5.0, (double) pos.getY() + (0.5 - random.nextDouble()), d1 + random.nextDouble() / 5.0);
            }
        }
    }
}
